...
class CircleContactGenerator : public cyclone::ParticleContactGenerator
{
public:
    /**
     * Holds a pointer to the particles we're checking for collisions with. 
...
unsigned CircleContactGenerator::addContact(cyclone::ParticleContact *contact, unsigned limit) const
{
    const static cyclone::real restitution = 0.0f;
    ...
    for (unsigned i = 0; i < BLOB_COUNT; i++)
    {
		...
		for (unsigned j = 0; j < BLOB_COUNT; j++)
		{
			...
			{
				// Check for penetration
				cyclone::Vector3 blobDiff = particles[i].getPosition() - particles[j].getPosition();

				// The blob is nearest to the start point
				if (blobDiff.squareMagnitude() < BLOB_RADIUS*BLOB_RADIUS * 4)
				{
					// We have a collision
					contact->contactNormal = blobDiff.unit();
					contact->contactNormal.z = 0;
					contact->restitution = restitution;
					contact->particle[0] = particles + i;
					contact->particle[1] = particles + j;
					contact->penetration = BLOB_RADIUS - blobDiff.magnitude() * 2;
					used ++;
					contact ++;
				}
			}
...
class BlobDemo : public Application
{
	...
	CircleContactGenerator circleContactGenerator;
...
world(PLATFORM_COUNT+BLOB_COUNT, PLATFORM_COUNT)
{
	...
	circleContactGenerator = CircleContactGenerator();
	circleContactGenerator.particles = blobs;
	world.getContactGenerators().push_back(&circleContactGenerator);
